...
# # Option B
# all_sources = []
# with open('./polyphony_for_martha_updated.csv') as file:
...
#         parenthesis = re.search(r"\((.*?)\)", name_item)
#         bracket = re.search(r"\[(.*?)\]", name_item)

#         if parenthesis is not None:
#             name_item_edit = re.search(r"(.*?)\(.*\)", name_item).group(1)
#             embedded_content = parenthesis.group(1)
...
#         if embedded_content in part_tags:
#             all_sources.append(name_item_edit)
#         else:
#             all_sources.append(name_item)

# num_all = len(all_sources)
# num_unique = len(set(all_sources))
